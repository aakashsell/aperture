...
Create both Compose deployments in the same Dokploy project and connect them to the `main` branch of this GitHub repository. Set `APP_DOMAIN=aperture-app.bazement.net` on the core deployment and `SITE_DOMAIN=aperture.bazement.net` on the site deployment; their Traefik labels route each hostname to its own public service on port `3000`. Set `POSTGRES_PASSWORD` and a random 32-byte-or-longer `JWT_SECRET` as secrets on the core deployment. Set `APP_URL=https://aperture-app.bazement.net` on the site deployment. Keep the database and API private; only the app UI and landing service are public. A persistent `aperture-data` volume stores Postgres data.
